...
const FormInput = ({ label, name, value, onChange }) => {
    ...
    const [unit, setUnit] = useState(unitPart || 'Ft');
    ...
    return (
        <div className="mb-32 relative">
            ...
            <div className="flex items-center justify-end mb-10">
                <button
                    className={`text-white font-bold py-2 px-4 rounded-l-lg ${unit === 'Ft' ? 'bg-purple-600' : 'bg-purple-300'}`}
                    onClick={() => handleUnitChange('Ft')}
                >
                    Ft
                </button>
                <button
                    className={`text-white font-bold py-2 px-4 rounded-r-lg ${unit === 'Cm' ? 'bg-purple-600' : 'bg-purple-300'}`}
